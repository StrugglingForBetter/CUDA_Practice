...
int main(int argc, char** argv) {
	...
	initialData(in_host, nxy);
	float temp1_[] = { -1.0, -2.0, 2.0, 1.0 };
	CHECK(cudaMemcpyToSymbol(coef, temp1_, TEMP_RADIO_SIZE * sizeof(float)));
	...
	convolution(in_host, out_cpu, temp1_, nxy);
	...
	CHECK(cudaMemcpy(dcoef_ro, temp1_, TEMP_RADIO_SIZE * sizeof(float), cudaMemcpyHostToDevice));
